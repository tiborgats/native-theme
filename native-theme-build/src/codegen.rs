...
///
/// Handles backslash, double-quote, newline, carriage return, tab, and null byte.
/// This is for string literals only, NOT format strings (does not escape `{`/`}`).
pub(crate) fn escape_rust_str(s: &str) -> String {
    ...
    for c in s.chars() {
        match c {
            ...
            '\r' => out.push_str("\\r"),
            '\t' => out.push_str("\\t"),
            // Issue 3: Escape null bytes for defense in depth
            '\0' => out.push_str("\\0"),
...
pub(crate) fn generate_code(
    ...
    // Issue 16: writeln! to a String is infallible (String's fmt::Write
    // implementation can always allocate). `let _ = ...` suppresses the
    // #[must_use] warning on the Ok(()) result.
    let mut out = String::new();

    // Issue 18: Pre-compute role -> PascalCase variant mappings once.
    let role_variants: Vec<(&str, String)> = config
        .roles
        .iter()
        .map(|r| (r.as_str(), r.to_upper_camel_case()))
        .collect();
    ...
    if crate_path == "native_theme" {
        // Issue 47: Suppress unused_extern_crates lint in 2021+ editions
        let _ = writeln!(out, "#[allow(unused_extern_crates)]");
    ...
    for (_role, variant) in &role_variants {
...
/// at least one non-default key that maps to a known DE variant AND where
/// the DE-specific values differ from the default.
///
/// Issue 53: When all DE overrides have the same value as the default, the
/// mapping collapses to a simple arm and no DE cache is needed.
        ...
        if let Some(mapping) = mappings.get(theme_name) {
            for value in mapping.values() {
                if let MappingValue::DeAware(de_map) = value {
                    let default_val = de_map.get("default");
                    let de_entries: Vec<_> = de_map
                        .iter()
                        .filter(|(k, _)| k.as_str() != "default")
                        .filter(|(k, _)| de_key_to_variant(k).is_some())
                        .collect();
                    if de_entries.is_empty() {
                        continue;
                    }
                    // Issue 53: Skip if all DE values equal the default
                    let all_same =
                        default_val.is_some_and(|dv| de_entries.iter().all(|(_, v)| *v == dv));
                    if !all_same {
                        return true;
                    }
                }
...
fn generate_icon_name(
    ...
    if needs_de_cache {
        ...
        // Issue 21: Document OnceLock caching behavior in generated code
        let _ = writeln!(
            out,
    ...
    for theme_name in all_themes {
        ...
        let icon_set = match theme_name_to_qualified_icon_set(theme_name, crate_path) {
            ...
            None => {
                debug_assert!(
                    false,
                    "unknown theme \"{theme_name}\" reached codegen (validation should have caught this)"
                );
                continue;
            }
        };
        if let Some(mapping) = mappings.get(theme_name) {
            for (role, variant) in role_variants {
                if let Some(mv) = mapping.get(*role) {
                    match mv {
                        ...
                        MappingValue::DeAware(de_map) => {
                            ...
                            // Issue 53: Collapse to simple arm when all DE overrides
                            // have the same value as the default.
                            let all_same_as_default = !de_entries.is_empty()
                                && de_entries
                                    .iter()
                                    .all(|(_, escaped)| *escaped == escaped_default);

                            if de_entries.is_empty() || all_same_as_default {
                                // No DE-specific overrides, or all values equal default
                                let _ = writeln!(
                                    out,
...
"#,
    ...
    // === Issue 3: escape_rust_str null byte ===

    #[test]
    fn escape_rust_str_null_byte() {
        assert_eq!(escape_rust_str("a\0b"), "a\\0b");
    }

    // === Issue 47: extern crate lint suppression ===

    #[test]
    fn generated_code_extern_crate_has_allow_lint() {
        let (config, mappings) = sample_config_and_mappings();
        let output = generate_code(&config, &mappings, "icons", "native_theme", &[]);
        assert!(
            output.contains("#[allow(unused_extern_crates)]"),
            "extern crate should have lint suppression. output:\n{output}"
        );
    }

    // === Issue 48: Replace tautological icon_svg_has_wildcard test ===

    #[test]
    fn generated_code_icon_svg_has_specific_match_arms() {
        let (config, mappings) = sample_config_and_mappings();
        let output = generate_code(&config, &mappings, "icons", "native_theme", &[]);
    ...
    // === Issue 49: Document generate_icons() limitations ===

    #[test]
    fn generated_code_simple_api_uses_default_crate_path() {
        // The simple API always uses "native_theme" as crate_path
        let (config, mappings) = sample_config_and_mappings();
        let output = generate_code(&config, &mappings, "icons", "native_theme", &[]);
        assert!(
    ...
    // === Issue 64: Default-only DeAware assert !OnceLock ===

    #[test]
    fn generated_code_de_aware_default_only_no_oncelock() {
        let (config, mappings) = de_aware_default_only_config_and_mappings();
        let output = generate_code(&config, &mappings, "icons", "native_theme", &[]);
        // DeAware with only a default key should NOT emit OnceLock
        assert!(
            !output.contains("OnceLock"),
            "DE-aware with only default should not emit OnceLock. output:\n{output}"
        );
    }

    // === Issue 10: Double slash fix ===

    #[test]
    fn generated_code_empty_base_dir_no_double_slash() {
        let (config, mappings) = sample_config_and_mappings();
        // Empty base_dir simulates TOML at manifest root
        let output = generate_code(&config, &mappings, "", "native_theme", &[]);
        // Should not contain double slash in include_bytes! path
        assert!(
            !output.contains("\"//"),
            "empty base_dir should not produce double slash. output:\n{output}"
        );
        // Should still have valid paths
        assert!(
            output.contains("material/play_pause.svg"),
            "should have valid SVG paths. output:\n{output}"
        );
    }

    // === Issue 21: OnceLock caching comment ===

    #[test]
    fn generated_code_de_aware_has_oncelock_comment() {
        let (config, mappings) = de_aware_config_and_mappings();
        let output = generate_code(&config, &mappings, "icons", "native_theme", &[]);
        assert!(
            output.contains("XDG_CURRENT_DESKTOP is read once and cached"),
            "should have OnceLock caching comment. output:\n{output}"
        );
    }
    ...
    #[test]
    fn generated_code_collapses_all_same_de_aware() {
        let config: MasterConfig = toml::from_str(
            r#"
name = "app-icon"
roles = ["play-pause"]
...
"#,
        ...
        let freedesktop_mapping: ThemeMapping = toml::from_str(
            r#"
play-pause = { kde = "play", gnome = "play", default = "play" }
"#,
        )
        .unwrap();
