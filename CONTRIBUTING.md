...
the feature-combination check (`scripts/check-features.sh`: every crate's
...
have examples, the widget-coverage check (`scripts/check-widget-coverage.py`,
...
(`scripts/compat-check.sh check`). The script prints
...
A scheduled workflow, `dependency-canary.yml`, runs nightly at 18:10 UTC
(and on demand from the Actions tab) with every dependency updated on a
throwaway lockfile.
...
```bash
./scripts/pre-release.sh        # full screenshots + GIFs pipeline (needs gh, spectacle, Python 3.11+ with Pillow, ImageMagick 7 and network access; see scripts/README.md)
