...
- [x] **ICON-03**: icon_theme field (`Option<String>`) on ThemeVariant with preset-specific default assignments in TOML
- [x] **ICON-04**: `icon_name()` function to look up the platform-specific identifier string for a given icon set and role
- [x] **ICON-05**: `system_icon_set()` function to resolve "system" to the OS-native icon set name (macOS->sf-symbols, Windows->segoe-fluent, Linux->freedesktop)

### Platform Loading

- [x] **PLAT-01**: macOS icon loading via `NSImage(systemSymbolName:)` -> rasterized RGBA pixels (feature "system-icons")
- [x] **PLAT-02**: Windows stock icon loading via `SHGetStockIconInfo` -> RGBA pixels (feature "system-icons")
- [x] **PLAT-03**: Windows Segoe Fluent Icons font glyph rendering for action/navigation/window roles (feature "system-icons")
...
- [x] **INTG-03**: gpui connector: IconData->RenderImage conversion + `icon_name()` Lucide shortcut for gpui-component IconName
- [x] **INTG-04**: iced connector: IconData conversion helpers
- [x] **INTG-05**: gpui example updated with icon display and icon set selector dropdown
...
| PLAT-02 | Phase 20 | Complete |
| PLAT-03 | Phase 20 | Complete |
