...
- [ ] **Step 2: `CHANGELOG.md`**: add the seven entries of spec §8 under the existing `## [Unreleased]` heading, alongside the canary / publish-job entries, using the file's own taxonomy: the two marked **Breaking Changes** go under a `### Breaking Changes` → `#### native-theme-gpui` group, exactly as v0.5.8 filed the move to 0.6.0 (`CHANGELOG.md:20-29`); the rest into `### Added`, `### Changed` and `### Fixed`. Do **not** date the heading or add a compare link: `pre-release-check.sh:593` makes the asset-stamp check a hard failure once `## [0.5.9] - <date>` exists; that edit belongs to the maintainer's release commit.
