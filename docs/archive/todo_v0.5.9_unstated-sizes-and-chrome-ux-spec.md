...
- The rule is for single-line controls. A multi-line Input sets its own height before the refinement (input.rs:706-709), which the refinement overrides today; the showcase's Textarea applies its own `Styled::h` after the builder, and the builder's doc says so.
