...
## 2. ResolvedThemeVariant — Non-Optional Output

After the full pipeline (OS reader → TOML overlay → resolve → app TOML → resolve),
convert `ThemeVariant` (with `Option` fields) into a `ResolvedThemeVariant`
...
```
  ...
  App TOML (opt) ─────▶ merge() — app overrides
                       ...
                       validate()
                              │
                              ▼
                       ResolvedThemeVariant (plain values)
                              │
                              ▼
...
The ResolvedThemeVariant mirrors the per-widget architecture from the
...
- **`validate()`** runs after all TOML overlays and resolve passes. It converts
  `ThemeVariant` → `ResolvedThemeVariant`, checking that every required
...
`&ResolvedThemeVariant` (or its sub-structs) instead of `&ThemeVariant`. This
...
### Step 2: Create ResolvedThemeVariant Module
...
Change connectors to accept `&ResolvedThemeVariant`. Remove all Option handling.
...
- **Unit**: `validate()` with all fields `Some` → `Ok(ResolvedThemeVariant)`
...
- **Serde**: ResolvedThemeVariant does NOT implement Deserialize (output only)
