...
| D43 | The base-theme observer also restores the 12 base-palette colours of the styled theme from the stored variant | `ThemeConfigColors` keeps them private, so the config a variant installs cannot carry them and `Theme::apply_config` resets them to upstream's constants on every rebuild; every rebuild ends in `cx.set_global` of `gpui_base::Theme` (`theme/mod.rs:247-256`, `:321-325`), so the existing observer sees it; the styled theme has no upstream observer, so the write cannot loop (final review, error 62) |
...
| Tab geometry | `tab/tab.rs:801-808` writes `h`, text size, `rounded` | `Styled for Tab` (`:606-608`) hands out gpui-base `Tab`'s style bag; render's own setters overwrite the caller's values in it before gpui-base applies it (`tabs.rs:180`); `min_width`/padding survive | let the caller's values stand |
...
implementation of Task 3, 56–57 from Task 5, 58 from Task 9, 59 from Task 10's review, 60–61 from Task 15's review, 62–64 from the final whole-branch review.
...
62. **`to_theme_config` claimed gpui-component derives the 12 private base-palette colours from `ThemeColor`.** It does not: `Theme::apply_config` passes `ThemeColor::dark()` / `light()` as the fallback and `apply_color!` writes that constant for every unset field (`schema.rs:687-695`, `:1074-1078`), so `Badge`, `Label` and `ColorPicker` reverted to upstream's palette on the first `Theme::change`. Found by the final review. Fixed at the root within the connector's reach: the observer restores the 12 from the stored variant (D43), with a test proving the reset happens without it; the durable fix (public fields) is on the upstream list.
63. **The 101-variant tripwire comment promised what the test cannot do.** `IconName` has no enumeration, so the hand-written list catches removals (compile error) but not additions. The comment, README and §14 now say so; the upstream request stands.
64. **The Tab correction named the wrong mechanism.** Errors 60's fix said the render re-sets the fields "after applying the refinement" / "after `refine_style`"; gpui-component's `Tab` never calls `refine_style`. Its `Styled` impl hands out gpui-base `Tab`'s style bag and its render's own setters overwrite the caller's values in that bag before gpui-base applies it. The conclusion (Tier U for height, radius, text size) stands; wording corrected everywhere.
