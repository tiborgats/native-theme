...
| CI installs `libxcb1-dev libxkbcommon-dev libxkbcommon-x11-dev libfontconfig1-dev libfreetype-dev` on every job that builds the connector (clippy matrix entry, connector test, workspace doc; the docs-site and crates.io workflows likewise). The 0.6 tree builds `yeslogic-fontconfig-sys` with default features (no `dlopen`), so its build script runs `pkg-config fontconfig` at check time; the 0.5 stack's CI ran without the package. | `.github/workflows/ci.yml:41, 98, 112` |
