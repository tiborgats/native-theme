...
| D43 | The base-theme observer also restores the 12 base-palette colours of the styled theme from the stored variant | `ThemeConfigColors` keeps them private, so the config a variant installs cannot carry them and `Theme::apply_config` resets them to upstream's constants on every rebuild; every rebuild ends in `sync_base`, so the existing observer sees it; the styled theme has no upstream observer, so the write cannot loop (final review, error 62) |
