...
- `font.family`, `font.size`, `font.weight`, `font.style`, `font.color` — individual font sub-properties, shown when at least one platform has a widget-specific value. `font.style` is a string enum: `Normal` (upright), `Italic`, or `Oblique`. `font.color` is the default-state text rendering color for this font.
...
3. **GNOME "sp" unit:** Some GNOME/libadwaita values are suffixed
   ...
   unit — it equals logical pixels at the default text scale factor
...
- `track_radius` — corner radius of the switch track. A value of half the `track_height` produces a pill/capsule shape.
...
- `indicator_width` — side length of the checkbox indicator (square) or diameter of the radio indicator (circle).
...
- `overlay_mode` — boolean: `true` if the scrollbar uses overlay (auto-hiding) mode, `false` if persistent/legacy. Platform cells show how to query this (e.g. `NSScroller.preferredScrollerStyle`).
...
- `body_font` — typeface for the primary body text (e.g. dialog message body). Includes `body_font.color`.
- `title_font.*` — typeface sub-properties for the title/heading text. Includes `title_font.color`.
- `title_bar_font.*` — typeface sub-properties for the window title bar text. Includes `title_bar_font.color`.
- `item_font` — typeface for list/table row content text. Includes `item_font.color`.
- `header_font` — typeface for list/table column header text. Includes `header_font.color`.
...
| `style`        | `NSFontDescriptor` traits → Normal  | **(none)** — preset: Normal           | `[General] fixed` style field | `monospace-font-name` → style |
| `color`        | ← `defaults.font.color`          | ← `defaults.font.color`              | ← `defaults.font.color`          | ← `defaults.font.color`          |
...
logical pixels (per the outer-box rule) — from the outside of the
...
have no platform limit — preset values are our defaults.
...
| `thumb_width`    | mode-dependent: legacy=16 (fills groove), overlay=7 (idle) / 11 (hover) | ↕ `SM_CXVSCROLL` (same)    | `ScrollBar_SliderWidth` = 8| **(Adwaita CSS)**: 8      |
...
| `border.shadow_enabled` | **(none)** — no shadow | **(none)** — no shadow | **(none)** — no shadow | **(none)** — no shadow |
| `border.padding_horizontal` | **(none)** — no window status-bar widget: AppKit's `NSStatusBar` manages status items "displayed within the system-wide menu bar" ([Apple docs](https://developer.apple.com/documentation/appkit/nsstatusbar)); HIG Status bars: "Not supported in macOS" ([HIG](https://developer.apple.com/design/human-interface-guidelines/status-bars)) | **(none)** — not specified: Win32 status bar borders are runtime-queryable **(API)** ([`SB_GETBORDERS`](https://learn.microsoft.com/en-us/windows/win32/controls/sb-getborders)), no default documented ([Status Bars](https://learn.microsoft.com/en-us/windows/win32/controls/status-bars)); WinUI3 has no status-bar control ([controls list](https://learn.microsoft.com/en-us/windows/apps/design/controls/)) | ✅ 2 left / 14 right — Qt `QStatusBar::reformat()`: layout margins `QMargins()` (all 0), `addSpacing(2)` before the items, nothing after them ([qstatusbar.cpp:418-474](https://github.com/qt/qtbase/blob/d865bf9d4877fa09e1fc02e2ad14fb9fd43ef430/src/widgets/widgets/qstatusbar.cpp#L418-L474)); then the size grip, enabled by default (:200-201; KXmlGui leaves it so), 1px after the items (:466-467) and 13px wide: its size hint is `QSize(13, 13)` through `CT_SizeGrip` ([qsizegrip.cpp:198-202](https://github.com/qt/qtbase/blob/d865bf9d4877fa09e1fc02e2ad14fb9fd43ef430/src/widgets/widgets/qsizegrip.cpp#L198-L202)), horizontal policy `Minimum` (:183), and neither Breeze (`sizeFromContents` → parent, [breezestyle.cpp:1155-1156](https://github.com/KDE/breeze/blob/f0b1d7534aa2356d7336241d0c7051522e8a6b68/kstyle/breezestyle.cpp#L1155-L1156)) nor `KStyle` ([kstyle.cpp](https://invent.kde.org/frameworks/frameworkintegration/-/blob/178c522c70c0c1295f59d4776596c8a361dec872/src/kstyle/kstyle.cpp), no `sizeFromContents`) changes it, `QCommonStyle` returning it as given ([qcommonstyle.cpp:5032-5035](https://github.com/qt/qtbase/blob/d865bf9d4877fa09e1fc02e2ad14fb9fd43ef430/src/widgets/styles/qcommonstyle.cpp#L5032-L5035)). Breeze paints the grip as nothing (`CE_SizeGrip` → `emptyControl`, [breezestyle.cpp:1505-1506](https://github.com/KDE/breeze/blob/f0b1d7534aa2356d7336241d0c7051522e8a6b68/kstyle/breezestyle.cpp#L1505-L1506)), so the items end 1 + 13 = 14px of empty bar before the right edge. Context: a window that is neither maximized nor full screen; in those two states the grip is hidden (qsizegrip.cpp:76-89, :450-463) and the right is 0. `showMessage()` text: 6 left / 12 right (`messageRect()`, :97-98). Breeze has no status-bar metric (`breezemetrics.h`) | ✅ **(Adwaita CSS)** 10 — `statusbar { padding: 6px 10px 6px 10px; }` ([_deprecated.scss:197-199](https://gitlab.gnome.org/GNOME/libadwaita/-/blob/a5030fc692855c62a0014ea784d3a80ce30f926f/src/stylesheet/widgets/_deprecated.scss#L197-199)); `GtkStatusbar` "Deprecated: 4.10" ([gtkstatusbar.c:83](https://gitlab.gnome.org/GNOME/gtk/-/blob/57f569a23cc23402eb122e337b69d57e4aeecd2b/gtk/deprecated/gtkstatusbar.c#L83)) |
| `border.padding_vertical` | **(none)** — no window status-bar widget (see `border.padding_horizontal`) | **(none)** — not specified (see `border.padding_horizontal`) | ✅ 3 top / 2 bottom — Qt `QStatusBar::reformat()`: `addSpacing(3)` above the items row, `addSpacing(2)` below it (qstatusbar.cpp:437, :472). Breeze's 1px top separator is painted inside the bar, not laid out (`drawPanelStatusBarPrimitive`) | ✅ **(Adwaita CSS)** 6 — same `statusbar` rule |
...
| `item_font.color`     | ← `defaults.font.color`               | ← `defaults.font.color`| `[Colors:View] ForegroundNormal`   | libadwaita `.list` fg       |
...
| `selection_background`           | ← `defaults.selection_background`                | ← `defaults.selection_background` | ← `defaults.selection_background`                | ← `defaults.selection_background`     |
| `selection_text_color`| ← `defaults.selection_text_color`      | ← `defaults.selection_text_color`| ← `defaults.selection_text_color`| ← `defaults.selection_text_color`|
| `header_font`         | ← `defaults.font`                     | ← `defaults.font`    | ← `defaults.font`                     | ← `defaults.font`          |
...
| `row_height`         | NSTableView row: 24                    | WinUI3 ListView: 40    | **(none)** — sizes to content          | **(Adwaita CSS)**: rich-list=32 (`.rich-list` row min-height), plain list=content-driven (no min-height) |
...
| `section_heading` | Section divider (settings group header)  | `.headline`: 13pt, **700** | Subtitle: 20epx, **600** (=15pt @96dpi) | Level 2: body × 1.20; `Font.Normal` (400) unless `type: Primary` (kirigami `8319acc` [controls/Heading.qml:20-21,32,35](https://invent.kde.org/frameworks/kirigami/-/blob/8319acc7dd1149664147ec1e22c39ae6143ed752/src/controls/Heading.qml#L20-L35); `type` defaults to `Normal`, [templates/Heading.qml:89](https://invent.kde.org/frameworks/kirigami/-/blob/8319acc7dd1149664147ec1e22c39ae6143ed752/src/templates/Heading.qml#L89))  | `.heading`: 11pt, **700**|
| `dialog_title`    | Dialog/page title (sheet header)         | `.title1`: 22pt, 400 | Title: 28epx, **600** (=21pt @96dpi) | Level 1: body × 1.35; `Font.Normal` (400) unless `type: Primary` (kirigami `8319acc` [controls/Heading.qml:17-18,32,35](https://invent.kde.org/frameworks/kirigami/-/blob/8319acc7dd1149664147ec1e22c39ae6143ed752/src/controls/Heading.qml#L17-L35); `type` defaults to `Normal`, [templates/Heading.qml:89](https://invent.kde.org/frameworks/kirigami/-/blob/8319acc7dd1149664147ec1e22c39ae6143ed752/src/templates/Heading.qml#L89))        | `.title-2`: ≈15pt, **800**|
...
| `body_font.color`    | ← `defaults.font.color`    | ← `defaults.font.color`    | ← `defaults.font.color`     | ← `defaults.font.color`     |
...
| `border.padding_horizontal`| ~8–10px **(measured)**   | WinUI3: 12 left / 0 right — `ComboBoxPadding` = 12,5,0,7 ([ComboBox_themeresources.xaml:341](https://github.com/microsoft/microsoft-ui-xaml/blob/2b8c7757ef2dd57234d5d7c3016d136c59a24bad/controls/dev/ComboBox/ComboBox_themeresources.xaml#L341)) on the text in column 0, which ends at the 38px arrow column ([:566-567](https://github.com/microsoft/microsoft-ui-xaml/blob/2b8c7757ef2dd57234d5d7c3016d136c59a24bad/controls/dev/ComboBox/ComboBox_themeresources.xaml#L566-L567)); the 12px chevron sits right-aligned in it with a 14px right margin ([:582](https://github.com/microsoft/microsoft-ui-xaml/blob/2b8c7757ef2dd57234d5d7c3016d136c59a24bad/controls/dev/ComboBox/ComboBox_themeresources.xaml#L582)) | `ComboBox_FrameWidth` = 6 left / 0 right — the edit field is inset by `PM_ComboBoxFrameWidth` on the left, top and bottom but not the right (`labelRect.adjust(frameWidth, frameWidth, 0, -frameWidth)`, [breezestyle.cpp:3009](https://github.com/KDE/breeze/blob/f0b1d7534aa2356d7336241d0c7051522e8a6b68/kstyle/breezestyle.cpp#L3009)), and ends at the 20px `MenuButton_IndicatorWidth` arrow column (`SC_ComboBoxEditField`, [:3005](https://github.com/KDE/breeze/blob/f0b1d7534aa2356d7336241d0c7051522e8a6b68/kstyle/breezestyle.cpp#L3005); `SC_ComboBoxArrow`, [:2990-2999](https://github.com/KDE/breeze/blob/f0b1d7534aa2356d7336241d0c7051522e8a6b68/kstyle/breezestyle.cpp#L2990-L2999)) | ← button padding (10px)     |
...
| Dialog button order: Windows primary leftmost (modern WinUI 3 continuity, verified 2026-04-20) | ✅ [MS WinUI 3 Dialog controls](https://learn.microsoft.com/en-us/windows/apps/design/controls/dialogs-and-flyouts/dialogs) (ms.date 2025-08-05, updated_at 2026-04-04): "PrimaryButton — Represents the first 'do it' action. Appears as the leftmost button." / "CloseButton — Represents the safe, nondestructive action that enables the user to exit the dialog. Appears as the rightmost button." Confirms continuity with Win7 Common Buttons convention. |
