# Iced Connector: Full Theme Geometry Support
...
For this document: the tables' `padding_horizontal`/`padding_vertical`
are four optional sides now, and a helper such as `tooltip_padding()`
would follow `button_padding` and `input_padding` — each side the theme's
where stated, iced's own default where not — rather than return an axis
value.
