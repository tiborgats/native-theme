...
- **MSRV** (§4.4): workspace floor re-measured after the refresh (expected `1.89.0`); connector gets its own `rust-version`, provisional `1.95`, measured in Task 13. Both are *measured*, never assumed.
...
- Produces (breaking, §7.1):
  ...
  - `pub(crate) fn text_scale(prefs: &AccessibilityPreferences) -> f32` (used by Task 11)
...
/// and positive, else `1.0` (spec §7.2).
pub(crate) fn text_scale(prefs: &AccessibilityPreferences) -> f32 {
...
mod apply_tests {
    ...
    fn native_unscaled_has_factor_one() {
        ...
        assert_eq!(text_scale(n.accessibility), 1.0);
...
pub fn control_height(
    ...
    let text = (font.size * text_scale(n.accessibility) * n.resolved.defaults.line_height).ceil();
