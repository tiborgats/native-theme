...
fn test_gnome_resolve_validate() {
    ...
    // (gsettings calls return None). Both icon_set and icon_theme are Theme-level
    // (shared) after the G4 migration; KDE remains the one exception with
    // per-variant icon_theme overrides.
