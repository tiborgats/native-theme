...
- The gpui showcase is an application, and its Widget Info describes the widget under the pointer rather than a demo block. The window's title bar is gpui-component's `TitleBar`, labelled `native-theme-gpui <version> showcase` — the title the OS shows too — with File, View, Theme and Help menus whose items act; five of their actions also have key bindings (Ctrl+Q quits, Ctrl+B and Ctrl+I toggle the Sidebar and the inspector, Ctrl+K opens the command palette and Ctrl+, the Preferences). Under it a toolbar drawn with `geometry::toolbar` holds buttons for the command palette, a theme reload and the Preferences. A `Sidebar` replaces the tabs, and its header holds the theme settings, each labelled above its control: Theme (the preset switch, a searchable `Combobox`), Mode (a System / Light / Dark `Select`) and Icon set (a `Select`). The Sidebar, the page and an inspector are the panels of one resizable group whose handles drag; and a `StatusBar` holds the Sidebar's toggle at its left end and the inspector's at its right, and between them names the desktop, the preset and mode, the theme's font in the unit its source stated, the text scale, the accessibility flags that are set, and the hovered widget. A command palette (Ctrl+K), a Preferences sheet for the four accessibility preferences and an About dialog open as overlays, and a theme that fails to load is reported by an `Alert`. Every widget on the pages and in the chrome reports its own info, headings and list rows included, with two exceptions: the resizable group and its panels do not yet (recorded in `docs/todo.md`), and the inspector's content below its TabBar reports nothing by design, so the pointer can move into it without replacing what it shows. The innermost one under the pointer is shown in the inspector once the pointer has rested on it for 250 ms: Theme colors (a swatch and the upstream line it was read at for each), Theme config, Not themeable, and a new This instance section for facts about the demo that no theme could set, with a Copy button for the text. The inspector's Theme tab holds the window-level facts. The example is a module tree, `examples/showcase-gpui/`, where it was one file of 8,732 lines.
...
- The gpui showcase's chrome is rearranged. The panel toggles moved from the two ends of the toolbar to the two ends of the status bar, beside the panels they control: small ghost buttons with the chosen set's `PanelLeft` and `PanelRight`, selected while their panel is open, with tooltips naming their keys. gpui-component's `SidebarToggleButton` draws its own icon whatever set is chosen, so it is no longer used and is an exception entry. The theme settings moved from the toolbar into the Sidebar's header, which the icon rail hides; the Sidebar stays 200px wide, which the header fits at text scales 1 and 2 because its three controls are triggers that truncate their text, and the colour mode's System row reads "System", the desktop's mode being in the status bar. The title bar, which named the preset and mode, and the OS window title both read `native-theme-gpui <version> showcase`, and the version left the status bar. Where the theme states no toolbar padding, the row takes `layout.container_margin` and, failing that, the showcase's own 8px, and its info says which. The Icons page gains an Icon Sizes section: the chosen set's icon at each of the five `defaults.icon_sizes` contexts, each reporting its own info and marking a native preset's size where platform-facts documents none.
...
- The gpui showcase's icons come from the set it names. Choosing gpui-component's built-in set showed placeholders for all 101 icons and the previous set's icons under the label "lucide", and a theme switch then loaded bundled Lucide in its place; the freedesktop loading spinner came from the system icon theme whichever theme the icons came from; a "fallback" label marked any system icon byte-identical to Material's, although the loaders never substitute; and `--icon-theme` without `--icon-set` named the theme it was given while the icons on show were the ones loaded before the flag was read. Each now loads, and names, one set.
...
- The gpui showcase's Sidebar icons overlapped on KDE. They were sized with `geometry::icon_size_panel`, which is 48px on kde-breeze, in rows 1.75 rem tall and a 48px icon rail; they take `geometry::icon_size_small`, 16px on every native preset.
