...
- native-theme-iced: a hovered *checked* checkbox and a hovered *selected* radio keep their accent fill. `checkbox.hover_background` is the hover of the unchecked box and is opaque in 14 of the 16 presets, so compositing it over the checked fill replaced the accent with a neutral grey on 28 of the 32 preset/mode combinations. The model states no hover for the checked state, and a state the platform does not state has no distinct appearance, so that status now emits exactly the checked fill, mark and border. `styles::toggler` was never affected — it has `switch.hover_checked_background` of its own.
- native-theme-iced: a *disabled* checkbox's check mark is `checkbox.disabled_text_color`, the one foreground the platform states for everything it dims, and the same field the disabled label already took. `checkbox.indicator_color` is a foreground **on the accent**, and a disabled box does not show the accent: `checkbox.disabled_background` replaces it for the checked and unchecked state alike. Over the 32 preset/mode combinations the disabled mark's contrast against its fill rises from a 1.03–1.35 band to 1.17–2.71; it is the platform's own pair in all 32 now. `radio::Status` has no disabled variant in iced 0.14, so a radio button is unaffected.
