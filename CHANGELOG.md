...
- Showcase self-tests in both connectors, run by a plain `cargo test`: both showcases are built, laid out and clicked headlessly (gpui's `TestAppContext`; `iced_test`'s `Simulator`). Every tab must lay out, every resizable group must leave its panels room to drag, every advertised control must change the state it claims to, and in iced every widget that has a style function must carry it.
- Both showcases render every widget their toolkit offers an application — gpui 110 shown and 25 excepted of 135, iced 28 modules plus the eight `iced_aw` widgets — and `scripts/check-widget-coverage.py` fails when an upstream release adds a widget that is neither shown nor listed, with a reason, in `docs/showcase-exceptions.toml`; `pre-release-check.sh`, CI and the nightly dependency canary all run it. The gpui showcase previously exercised 15 of the connector's 34 geometry builders and rendered no `StatusBar`, `TitleBar` or `Combobox`; a test now requires every builder to be used. The iced showcase takes its spacing from the platform's `LayoutTheme` and its widget sizes from the builders iced offers (`Checkbox::size`, `Toggler::size`, `ProgressBar::girth`, the rule's thickness).
- The gpui showcase's "Widget Info" panels are checked rather than argued. Five tests read `showcase-gpui.rs` and fail on it: every demo block carries a panel and every block id is a `tt-` id (107 blocks, 107 panels); every `geometry::` builder a demo applies is named in that demo's panel; **every colour claim carries the upstream line it was read at, and the test opens that line and requires the field the claim names to be read there** — 369 claims, all cited; and every `file.rs, Symbol` in a panel's prose still resolves to something that exists. A sixth prints rather than fails (a panel legitimately says nothing about a state its demo does not show): the omission report measures each panel against the theme fields the files it cites actually read, and names the ones **no panel anywhere** mentions. That residual is **11**, and each one is a state the showcase does not demonstrate — the outlined Danger, Info, Success and Warning buttons' hover and pressed fills, a dock's drop target, the segmented tab bar, and a table footer. All six locate the vendored sources with `cargo metadata` from inside the test, so there is nothing to wire into a script and nothing that can be skipped; the earlier objection that a test cannot reach a dependency's source was simply wrong.
- Showcase demos for `Command`, `Select` and `Textarea`, and Widget Info panels for the two icon grids (`tt-native-icons`, `tt-animated-icons`). The `Command` palette had never been rendered; `Select` now sits beside `Combobox`, which is where the one difference between them is visible — the connector gives `Select` the platform's font colour and `Combobox` only its size and weight, because upstream's disabled colour arrives through the same call and a carried colour would beat it on the one and yield to it on the other.
...
- `geometry::tooltip_content`: the platform's `tooltip.max_width` less the bubble's two horizontal paddings and upstream's 1 px border, for the element an application passes to `Tooltip::element`. It is the one place that width makes a tooltip's text wrap, and it closes the overflowing tooltip below.
...
- The same panels' **"Not themeable"** section was a claim nobody had checked, and checking it is the same question as "could the connector do something about it". Sixty-three entries audited, **thirty-one of them wrong**. A `Button`'s font weight was called hardcoded on ten panels and was simply not carried — `geometry::button` now applies it. A `List`'s item height was called "hardcoded per Size" while its delegate applies `geometry::list_item`, which sets it; a disabled `Button` was said to use "the same variant colors at reduced opacity", where upstream uses the variant's token at 0.15 for the fill and `muted_foreground` at 0.5 for the text, neither of them the platform's three modelled disabled fields. `Button Sizes` and `TabBar` called their padding a property of the `Size` enum, where upstream re-applies the caller's refinement *after* the size arm, so a refinement wins. And four "animation: hardcoded" notes were wrong in the same way: `Theme::motion` is a writable field of twelve tokens that at least eighteen widgets read, and the connector overwrites it with the default because native-theme models no motion — our gap, not upstream's, and the notes now say which. Where a property really has no receiver the note now says **Tier U** — the platform states it, the model carries it, upstream offers nowhere to put it — instead of "hardcoded", which reads as *nothing can be done*: a `Link`'s underline and its hover and pressed text are the clearest case, since `link_hover` and `link_active` exist, the connector writes them, and a `Button::link()` reads them while the `Link` widget computes its own opacities instead. Six others simply described upstream wrongly: a `Tree`'s indent and disclosure chevron are in neither the demo nor `tree.rs` — `Tree::new` takes a `render_item` closure and draws no row content at all; a `DropdownButton`'s arrow is a `Caret` whose colour *is* themed, the button variant's text colour at 75%; an `Alert`'s variant icon is a default `Alert::icon` replaces; a `Tooltip`'s delay belongs to the element that carries it; a `Sidebar`'s 255px is a fallback the caller's own width displaces; and a `DataTable`'s row height has an escape hatch, `Size::Size(px)`, that changes the row height and nothing else. Notes gained `file.rs, Symbol` citations as they were corrected, so the existing gate now holds them to upstream too, A third batch found the mistake behind several of the others at once: gpui-component sets the rem to `Theme::font_size` (`root.rs:582`), which this connector fills from the platform's font, so a `rems()` or a `text_base()` upstream is *already* proportional to the platform. Six Button panels had called their label size Tier U when it is `button_text_size` — a fixed ratio of the platform's body text; the `Headings` panel listed px figures that were only its rem ladder at a 16px rem, which no bundled preset produces; and an `Editor`'s line height is what the widget sets *first*, upstream applying the caller's refinement last and saying in a comment that this is deliberate. Notes gained `file.rs, Symbol` citations as they were corrected, so the existing gate now holds them to upstream too, The 70-entry "unmodelled widget" bucket then collapsed to one mechanism: Alert, Tag, Badge, Kbd and Breadcrumb are each `Styled` and apply the caller's refinement *after* their own paddings, so upstream was never the obstacle — native-theme simply states no such widget. Two plain falsehoods fell out with it: a `Kbd` was said to use a monospace font where `kbd.rs` sets no family at all, and a `Label`'s "font weights: hardcoded" turned out to be the reverse — nothing sets one. The builders that carry a font spec carry its weight too, but gpui-component's `Theme` has no font-weight field and `Root::render` sets none, so text *outside* a builder-styled element — a plain `Label`, a heading, anything an application draws itself — renders at gpui's default weight rather than the platform's. What the audit found that is *ours* to fix — motion, the unwritten font weight, the unmodelled widgets — is filed in `docs/todo.md` rather than fixed here.
...
- The gpui showcase's Widget Info panel takes all the height the sidebar has left. It sized itself to its text instead: its textarea grew with the content up to thirty rows and carried that as a minimum height, so a long hover note pushed the panel past the bottom of the sidebar and scrolled inside itself while the sidebar scrolled too, with the space under it unused. The panel is now the sidebar column's only growing child — the controls above it keep their natural height — and its own scrollbar appears only once the text is taller than the room there is.
- The gpui showcase framed its List and its Tree with a square grey box of its own while the Table beside them was rounded: upstream's `DataTable` draws its own border from the theme's radius, and `List` and `Tree` draw none, so the frame around them was the application's — here a 1 px box in a colour no theme states. Both now take the new `geometry::list`, so all three agree under every preset. Every other box, card and bordered row the showcase draws for demonstration goes through one helper that reads `Theme::border`, `Theme::radius` and the platform's `defaults.border.line_width`; the placeholder fills, the reduced-motion labels and the error banner read their colours from the theme; and a test beside the builder-coverage test lexes the showcase and fails on any radius helper, any radius written as a number and any colour setter given a colour constructor. Twenty-five such values were in the file.
- A wheel turned inside the gpui showcase's List, Tree, scrollbar demo, Settings page, code editor, message scroller, data table or Widget Info panel no longer scrolls the page behind it as well. gpui dispatches a scroll wheel to every scroll container under the pointer and stops at none of them, and no gpui-component widget takes itself out of that list, so both moved by the same delta. Each demo box now takes the pointer out of its ancestors' hit test. The underlying upstream gap, and what a consumer can and cannot do about it, is recorded in `docs/todo.md`.
