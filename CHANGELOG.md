...
- native-theme: `dialog.title_font` is stated where platform-facts documents it (§2.22, `:1496-1497`): on macos-sonoma 13pt Bold, the emphasized system font (macos-sonoma-live states the weight, and the macOS reader the system font's size), and on adwaita and adwaita-live `.title-2`'s 15pt ExtraBold (800). Neither stated one, so a dialog's title took the body font: 13pt 400 on macOS, 11pt 400 on GNOME.
