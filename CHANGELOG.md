...
- **Icon-theme detection reports its failure; the application decides.** `system_icon_theme()`, `detect_icon_theme()` and `DetectionContext::icon_theme()` return `Result<String>` where they returned a name (`DetectionContext::icon_theme()` an `Arc<String>`). On Linux a failed detection is `Error::ReaderFailed` with reader `"icon-theme"`, whose message names each source tried and why it gave no name — the config directory unresolved (`$XDG_CONFIG_HOME` and `$HOME` unset), a file unreadable or without the key, a command not found, exiting unsuccessfully or printing nothing — and on an unrecognised desktop both attempts, KDE's and GNOME's; on a platform other than Linux, macOS, iOS and Windows it is `Error::PlatformUnsupported`. The cache keeps a failure as it keeps a name, so a failed detection is not repeated on every call, and `invalidate_caches()` clears it. `Resolved::icon_theme` and `SystemTheme::icon_theme` are `Option<Cow<'static, str>>`: `None` where the theme states no icon theme and detection fails, and the resolution does not fail over it. `ResolutionContext::from_system` puts `None` in `icon_theme` where detection fails. `FreedesktopLoader::load` and `FreedesktopLoader::load_indicator` with no theme set search no theme then: `None`, except a custom `IconProvider`'s own freedesktop SVG, which depends on no theme. `IconSetChoice::System` displays as `system (unavailable: <reason>)`.
...
- `scaled_text_size(size, &prefs)` in both connectors: a text size from the resolved theme times the user's text-scaling factor, for the sizes the connector does not apply itself — a widget's font in iced, a `text_scale` role's in either. The factor applies to every text size alike; `font_size` and `mono_font_size` (iced) are it for the default fonts, and the gpui `geometry` builders now scale through it.
...
- The iced showcase's text roles follow their definitions in `docs/platform-facts.md` §2.19. A page title was set in `display`, the hero text KDE does not have, which resolves there to the body size, so on kde-breeze each page's title was smaller than its description; page titles now take `dialog_title`, section titles `section_heading` where they took `dialog_title`, and the descriptions, readouts and instructions that borrowed `section_heading` the theme's body font. Every role now brings its weight with its size, the Markdown sample's `h1` and `h2` included, and `display` appears only in the text-scale table. Every text-bearing widget in it now takes its size, and its weight where the widget takes a font, from the theme: button labels `button.font`, fields `input.font`, checkboxes and radios `checkbox.font`, pickers `combo_box.font`, menus `menu.font`, tabs `tab.font`, tooltips `tooltip.font`, table headers and cells `list.header_font` and `list.item_font`, and other text `defaults.font`, where most of them had iced's 16px. iced fixes its default text size when the renderer is created, so `Settings::default_text_size` could not follow a theme switch; the iced connector's README and `font_size` docs now say so and name the receivers, and that the OS text-scaling factor applies to every text size alike, which the showcase now does for all its text; it had applied it to none.
- The iced showcase draws its text in the theme's font family, where it drew every text in iced's generic sans-serif family with only the theme's weight: role text in `defaults.font`'s family, widget text in its widget font's, and the Markdown sample's code in `defaults.mono_font`'s. iced 0.14 draws text with cosmic-text 0.15, which takes a face only at the exact weight asked for, and fontdb 0.23 files a variable font at its OS/2 weight alone, so a weight its family had no face of fell through to the next fallback family that had one; cosmic-text's macOS fallback list puts the system family `.SF NS` first and the monospace Menlo second, and the macOS screenshot showed bold section titles in a monospace bold. Where a variable face of the family covers the weight on its `wght` axis, the showcase now files a face at that weight over the same font data, and cosmic-text draws the platform's own font at the true weight; where none does, the text stays in its family at the nearest face iced can ask for, and the text-scale table and the Widget Info rows say which weight is drawn and why. A family the font database does not hold, or holds only at weights iced cannot ask for, is drawn in the generic family, and the Widget Info rows, the inspector and the status line then name the generic family and what it resolves to instead of claiming the theme's. The iced connector's README and *Font Configuration* docs show how to build the `iced::Font` with the theme's family, explain the exact-weight matching, and describe filing faces for a variable family's weights; they had taught converting the family to a `String`, which iced's `Family::Name` cannot take.
- The iced showcase's page tab strip lays its horizontal scrollbar out below the tabs: under an overlay-scrollbar theme such as macos-sonoma it floated over the tab labels and covered them wherever the tabs overflowed the window, since iced draws an overlay scrollbar at all times, not only while scrolling.
- native-theme: a failed icon-theme detection no longer names a theme. Since 0.3, where the freedesktop loader's own detection fell back to `"hicolor"`, and through 0.5.8, a failed detection named a theme: from 0.3.2 `system_icon_theme()` returned `"hicolor"` from every Linux source that found nothing — no config directory, no `kdeglobals` or `lxqt.conf` key, `gsettings` or `xfconf-query` missing, failing or printing nothing — and `"material"` on other platforms than Linux, macOS, iOS and Windows, so a failed detection made `hicolor`'s application icons, or the bundled Material set, the system's icon theme, which the resolver, `FreedesktopLoader` and both showcases then used as if detected. The failure is now reported (see `Breaking Changes`); with no theme stated, no theme's icons load, and both showcases say the system icon theme is unavailable and why.
- native-theme: a freedesktop icon comes only from the chosen theme or a theme its `Inherits=` chain declares. `freedesktop-icons` 0.4.0 looks a missing icon up in `hicolor`, then takes a loose file from an icon base directory or `/usr/share/pixmaps`, and looks in `hicolor` from the start when the theme is not installed, so `FreedesktopLoader::load`, `load_indicator` and a custom icon's freedesktop name could return another set's icon (on CachyOS the name `cachyos` looked up in breeze gave the loose `/usr/share/icons/cachyos.svg`, and an application icon breeze lacks gave `hicolor`'s). Such a result is now `None`, and so is every icon of a theme that is not installed; `hicolor` stays allowed when it is the theme asked for. The chain is now searched to its end, depth-first in declared order as the Icon Theme Specification's lookup does, where freedesktop-icons searched only the theme's direct parents: `default` inherits Adwaita, which inherits AdwaitaLegacy, and AdwaitaLegacy's icons were not found for `default`. `is_freedesktop_theme_available` and `list_freedesktop_themes` read the icon directories the lookup searches, `~/.icons` included and a relative `$XDG_DATA_DIRS` or `$XDG_DATA_HOME` entry ignored, so a theme they report is one the lookup finds.
