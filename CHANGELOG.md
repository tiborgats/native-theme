...
- `geometry::toolbar`: a toolbar row the application draws itself — gpui-component has no toolbar widget — from the model's `toolbar`: `bar_height` as a minimum height, `item_gap` between the items, `border` padding, `background_color`, and `font` size and weight. The height is a floor, not a height, because KDE's toolbar sizes to its content; there is no edge, because no platform states one. The model's toolbar had been read by nothing in either connector.
...
- The gpui showcase is an application, and its Widget Info describes the widget under the pointer rather than a demo block. The window's title bar is gpui-component's `TitleBar`, with File, View, Theme and Help menus whose items act; five of their actions also have key bindings (Ctrl+Q quits, Ctrl+B and Ctrl+I toggle the Sidebar and the inspector, Ctrl+K opens the command palette and Ctrl+, the Preferences). Under it a toolbar drawn with `geometry::toolbar` holds the Sidebar's toggle, the preset switch (a searchable `Combobox`), a System / Light / Dark `ToggleGroup`, the icon-set `Select`, and buttons for the command palette, a theme reload and the inspector. A `Sidebar` replaces the tabs; the Sidebar, the page and an inspector are the panels of one resizable group whose handles drag; and a `StatusBar` names the desktop, the preset and mode, the theme's font in the unit its source stated, the text scale, the accessibility flags that are set, and the hovered widget. A command palette (Ctrl+K), a Preferences sheet for the four accessibility preferences and an About dialog open as overlays, and a theme that fails to load is reported by an `Alert`. Every widget on the pages and in the chrome reports its own info, headings and list rows included (the resizable group and its panels do not yet; recorded in `docs/todo.md`), and the innermost one under the pointer is shown in the inspector once the pointer has rested on it for 250 ms: Theme colors (a swatch and the upstream line it was read at for each), Theme config, Not themeable, and a new This instance section for facts about the demo that no theme could set, with a Copy button for the text. The inspector's Theme tab holds the window-level facts. The example is a module tree, `examples/showcase-gpui/`, where it was one file of 8,732 lines.
...
- Documentation of `geometry::input_height` and the README's builder table: `Input::h` sets the height of a multi-line input only (`input/input.rs:706-709`); a single-line input takes it through `Styled::h`.
