...
- **Development**: mapping-contract tests, a contrast invariant and showcase self-tests in both connectors, rendered seam tests in the gpui connector, a documented-sizes gate, a nightly dependency canary and `scripts/check-features.sh` (every crate in every feature combination).
