...
- The iced showcase draws its text in the theme's font family, where it drew every text in iced's generic sans-serif family with only the theme's weight: role text in `defaults.font`'s family, widget text in its widget font's, and the Markdown sample's code in `defaults.mono_font`'s. A weight is asked for only where the family has a face of it; otherwise the text is drawn at the family's nearest face. iced 0.14 draws text with cosmic-text 0.15, which takes a face only at the exact weight asked for and sees a variable font at its default weight alone, so a weight its family has no face of fell through to the next fallback family that has one: on macOS, where the system family `.SF NS` comes first in that list and the font database holds no bold face of it, bold section titles were drawn in the monospace Menlo Bold, second in the list. A family the font database does not hold is drawn in the generic family, at a weight of the family that resolves to.
