...
- The iced showcase's push buttons take the theme's padding through `button_padding`: the pane controls and the card's two buttons used the showcase's own spacing, and the tooltip triggers iced's default. Its Widget Info no longer calls button and input padding "not themeable" — they are `button_padding` and `input_padding` — and its Theme Information shows the font sizes it draws, where under a preset it showed a size scaled by the OS text-scaling factor that no text in it is drawn at.
