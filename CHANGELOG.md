...
- iced's `secondary.base` colour now carries the platform's **placeholder** colour instead of its button surface, and `secondary.strong` is a copy of it. iced reads that slot as placeholder text in three widgets, and the placeholder was between 1.06:1 and 1.84:1 — invisible — on every preset in both modes. Applications that relied on `button::secondary` carrying the platform surface call `styles::button`, which carries idle, hover, pressed and label together.
