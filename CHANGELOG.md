...
- The showcase's InputGroup "Copy" button did nothing: it was built without an `on_click` handler, so it rendered and hovered but never ran. It now copies the field's text to the clipboard and confirms with a notification. Its flat, borderless look is upstream's own design for an in-group addon button and is left as upstream draws it.
