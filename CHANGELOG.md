...
- The gpui showcase's preset switch labelled `default` with `platform_preset_name`'s guess while the status bar named the preset the pipeline settled on; both now name the latter. A theme that failed to load put its own name in place of the installed theme's in the status bar and the preset switch, and one that failed to resolve its layout in place of the installed theme's too; both now stay the installed theme's.
