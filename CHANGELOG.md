...
- native-theme: a failed icon-theme detection no longer names a theme. From `system_icon_theme()`'s introduction in 0.3.2 through 0.5.8, every Linux source returned `"hicolor"` when it found nothing — no config directory, no `kdeglobals` or `lxqt.conf` key, `gsettings` or `xfconf-query` missing, failing or printing nothing — and other platforms than Linux, macOS, iOS and Windows returned `"material"`, so a failed detection made `hicolor`'s application icons, or the bundled Material set, the system's icon theme, which the resolver, `FreedesktopLoader` and both showcases then used as if detected. The failure is now reported (see `Breaking Changes`); with no theme stated, no icons load, and both showcases say the system icon theme is unavailable and why.
