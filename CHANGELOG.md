...
- A gate in native-theme (`presets/documented_sizes.rs`) that the native presets state every size their platform documents: every padding side, the toolbar's `bar_height` and `item_gap`, the menu's and the list's `row_height`, and the combobox's `arrow_area_width` where the platform gives one number, that `docs/platform-facts.md` gives for KDE, GNOME, macOS and Windows — 72 rows, one per platform and widget, each citing its platform-facts lines, and `None` where the platform documents nothing. Each row is checked in both variants against the full preset and against the live path (the full preset, its `-live` preset over it, and the reader's size constants over that), so a reader constant is held to the facts as well; the Windows and KDE constants moved out of their OS and feature gates so every test run sees them. Companion tests keep one row per platform and widget, keep each citation on the row it names, require a full preset and its `-live` twin to state the same sizes, and require the colour-scheme presets and ios to state none of them.
- Three more seam tests: under every native preset at its own platform's DPI, a real `Input`, `Select` and `Combobox` draw their content inset by the stated left padding, and a real `Button`, `Input`, `Select`, `Combobox`, `ListItem` and an application-drawn menu row are their stated height at text scale 1 and hold their text at 1, 1.1 and 2; under kde-breeze and windows-11, which state an arrow column, a real `Select` and `Combobox` keep upstream's own right inset, and without the column they draw the stated right side.
...
- The gpui showcase's shortcuts use gpui's `secondary` modifier — Cmd on macOS, Ctrl elsewhere — as gpui-component's own bindings do, so its menus show each platform's own; they were Ctrl everywhere.
...
- native-theme-iced: `to_theme`'s doc listed `Tooltip` among the widgets iced gives a `Catalog` implementation; a tooltip has none and is styled as a container (`tooltip.rs:70`). The crate doc said `default-features = false` keeps the metric helpers, but `button_padding` and `input_padding` read iced's default padding from `iced_widget` and need `widgets`; the feature table now says so.
- The iced showcase's buttons take the theme's padding: the push buttons `button_padding` (the pane controls and the card's two buttons used the showcase's own spacing, the tooltip triggers iced's default), the menu bar's, menus' and context menu's items `menu.border.padding` and the page tabs `tab.border.padding`, each over iced's button default for a side the theme leaves unstated. The `iced_aw` `Card`, `TabBar` and `Tabs` take `card.border.padding` and `tab.border.padding` where the theme states every side, and keep `iced_aw`'s own otherwise, and the containers dressed as a card take `card.border.padding` over a container's zero; they used the showcase's own spacing, and the Widget Info said the theme states no card padding (windows-11 states 12). Its Widget Info no longer calls button and input padding "not themeable" — they are `button_padding` and `input_padding` — and its Theme Information shows the font sizes it draws, where under a preset it showed a size scaled by the OS text-scaling factor that no text in it is drawn at.
...
- The gpui showcase's preset switch labelled `default` with `platform_preset_name`'s guess while the status bar named the preset the pipeline settled on; both now name the latter. A theme that failed to load put its own name in place of the installed theme's in the status bar and the preset switch, and one that failed to resolve its layout in place of the installed theme's too; and the failure ran upstream's mode switch, which rebuilt the palette from its stored configs — recomputing derived colours such as the buttons' hover and active shades, and showing upstream's own colours where the other mode had no native variant stored. A failed install now leaves the window as the installed theme drew it: its name, layout, mode and every colour, the colour-mode Select included.
- Hiding the gpui showcase's side panel left the info of a widget in it on show in the status bar; it is cleared as on a page change. The toolbar's buttons sat flush where the theme states no `toolbar.item_gap` (the colour-scheme presets and ios since the gap became optional); they are `layout.widget_gap` apart, and where that is unstated too, `TOOLBAR_GAP`, the showcase's own 4px.
